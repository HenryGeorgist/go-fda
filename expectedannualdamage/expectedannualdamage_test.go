...
func Test_EAD(t *testing.T) {
	...
	flowDists := make([]statistics.ContinuousDistribution, len(stages))
	for i, f := range flows {
		dist, _ := statistics.InitDeterministic(f)
		flowDists[i] = dist
	}
	//create a rating curve based on fake data
	//this is backwards from how i would like it
	rc := ratingcurves.RatingCurve{Curve: paireddata.UncertaintyPairedData{Xvals: stages, Yvals: flowDists}}

	//create some basic data for damages
	damages := []float64{10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 110.0, 120.0}
	stageDists := make([]statistics.ContinuousDistribution, len(damages))
	for i, s := range stages {
		dist, _ := statistics.InitDeterministic(s)
		stageDists[i] = dist
	}
	//this is backwards from how i would like it
	dc := stagedamagecurves.StageDamageCurve{Curve: paireddata.UncertaintyPairedData{Xvals: damages, Yvals: stageDists}}
